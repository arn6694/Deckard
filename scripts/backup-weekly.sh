#!/bin/bash
#
# Weekly Backup Script - Zeus to OMV
# Runs every Sunday at 1:00 AM
# Retention: 28 days (4 weeks)
# Creates weekly backups with hard-link deduplication to daily backups
#
...
LOG_FILE="/var/log/backup-weekly.log"
RETENTION_DAYS=28
BACKUP_TYPE="weekly"
DATE=$(date '+%Y-%m-%d')
...
# Start backup
log "=========================================="
log "Weekly Backup Starting (${TIMESTAMP})"
log "Destination: ${BACKUP_DEST}/${BACKUP_NAME}"
log "Retention: ${RETENTION_DAYS} days (4 weeks)"
...
# Get most recent daily backup for hard-link deduplication
LINK_DEST=""
if [ -d "${BACKUP_DEST}" ]; then
    LINK_DEST=$(find "${BACKUP_DEST}" -maxdepth 1 -type d -name "*_daily" -printf '%T@ %p\n' | sort -rn | head -1 | cut -d' ' -f2-)
fi
...
find "${BACKUP_DEST}" -maxdepth 1 -type d -name "*_weekly" | while read -r old_backup; do
